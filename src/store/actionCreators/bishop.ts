...
export function bishop(
  ...
  for (let i = 1; i < 8; i++) {
    ...
    if (+cell.cell[1] + i < 9 && cell.cell[0].charCodeAt(0) + i < 105) {
      resCell =
        String.fromCharCode(cell.cell[0].charCodeAt(0) + i) +
        (+cell.cell[1] + i);
  ...
  for (let c of board) {
    ...
    const isPiece: boolean = king
      ...
      : c.piece === null;
    if (isRightDiagonal && isPiece) arrOfRightDiagonalWithPieces.push(c.cell);
    if (isLeftDiagonal && isPiece) arrOfLeftDiagonalWithPieces.push(c.cell);
  }
  for (let i = 1; i < 8; i++) {
    let nexCell =
      String.fromCharCode(cell.cell[0].charCodeAt(0) + i) +
